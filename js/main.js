...
function renderAbout() {
  ...
  document.getElementById('academicTimeline').innerHTML =
    PORTFOLIO_DATA.academicTimeline.map(item => `
...
function renderSkills() {
  ...
  // 1. Génération des cartes Hard Skills avec la structure des barres de progression
  document.getElementById('hardSkillsGrid').innerHTML =
    PORTFOLIO_DATA.hardSkills.map((cat, i) => `
      <div class="bento-card ${cat.size === 'wide' ? 'wide' : ''}" data-index="${i}">
        ...
        <div class="bento-tags">
          ${cat.skills.map(s => `
            <div class="skill-item-progress" style="width: 100%; margin-bottom: 12px;">
              <div class="skill-info" style="display: flex; justify-content: space-between; margin-bottom: 4px; font-size: 0.9rem;">
                <span class="skill-name" style="color: var(--text-primary); font-weight: 500;">${s.name}</span>
                <span class="skill-percentage" style="color: var(--accent-steel); font-family: var(--font-mono); font-weight: 600;">${s.level}%</span>
              </div>
              <div class="progress-bar-bg" style="width: 100%; height: 6px; background: rgba(255,255,255,0.05); border-radius: 4px; overflow: hidden; border: 1px solid var(--border-color);">
                <div class="progress-bar-fill" data-level="${s.level}" style="width: 0%; height: 100%; background: var(--accent-steel); transition: width 1s cubic-bezier(0.4, 0, 0.2, 1); border-radius: 4px;"></div>
              </div>
            </div>
          `).join('')}
        </div>
  ...
  // 2. Gestion du clic : Ouvre la carte + Anime les barres de progression
  document.querySelectorAll('.bento-card').forEach(card => {
    card.addEventListener('click', () => {
      // On bascule l'état ouvert/fermé de la carte
      const isOpen = card.classList.toggle('open');
      
      // On sélectionne toutes les jauges à l'intérieur de CETTE carte
      const bars = card.querySelectorAll('.progress-bar-fill');
      
      bars.forEach(bar => {
        // Si la carte s'ouvre, on anime jusqu'au pourcentage cible, sinon on remet à 0%
        const targetLevel = bar.getAttribute('data-level');
        bar.style.width = isOpen ? `${targetLevel}%` : '0%';
      });
    });
  });

  // 3. Génération des Soft Skills (Inchangé)
  document.getElementById('softSkillsGrid').innerHTML =
    PORTFOLIO_DATA.softSkills.map(cat => `
      <div class="soft-card">
...
function renderProjects() {
  ...
  document.getElementById('projectsSecondary').innerHTML = rest.map(p => `
...
function renderDistinctions() {
  document.getElementById('distinctionsList').innerHTML =
    PORTFOLIO_DATA.distinctions.map(d => `
...
function renderLangInterests() {
  document.getElementById('languagesList').innerHTML =
    PORTFOLIO_DATA.languages.map(l => `
      <div class="lang-item" data-percent="${l.percent}">
        <div class="lang-top">
  ...
  document.getElementById('interestsList').innerHTML =
    PORTFOLIO_DATA.interests.map(i => `<span class="interest-pill">${i}</span>`).join('');
}
...
function renderContact() {
  ...
  document.getElementById('contactLinks').innerHTML = links.map(l => `
    <a class="contact-link" ${l.href ? `href="${l.href}"` : ''} ${l.href && l.href.startsWith('http') ? 'target="_blank" rel="noopener"' : ''}>
